...
this.dispatch = function(req, res) {
  ...
  if (typeof arr != 'undefined' && arr[arr.length-1] == "json") {
    format = 'json';
  }
  ...
  var parameters = {};
  fs.appendFile("./logs/log.txt", JSON.stringify(requestedUrl) + "\n\n", function(err){
    if (err) throw err;
    console.log("saved to file!");
  });
  util.extractPostData(req, function(post) {
    fs.appendFile("./logs/log.txt", JSON.stringify(post) + "\n\n", function(err){
      if (err) throw err;
      console.log("saved to file!");
    });         
  });
  if (requestedUrl.pathname == '/') {
    ...
    */
    ...
    switch (controller_name) {
      ...
      case "map": 
        callController(controller_name, "index", parameters);
        break;
      ...
      case "data":
        if (requestedUrl.pathname.match("/data/newData") && req.method == "POST") {
          ...
          util.extractPostData(req, function(post) {
            ...
            Device.findById(parameters['device_id'], function(err, device)) {
              console.log(device.data.ignore_post_requests);
              if (!device.data.ignore_post_requests) {
                util.log("Ignored post request for device with ID " + device.data.id);
                callController(controller_name, action, parameters); 
              }
            }
      ...
      case "home":
        if (requestedUrl.pathname.match("/home/documentation") && req.method == "GET") {
          action = "documentation"; 
          callController(controller_name, action, parameters);
        } else if (requestedUrl.pathname.match("/home/about") && req.method == "GET") {
          action = "advert";
          callController(controller_name, action, parameters);
        }
      break;
      ...
      case "mobile":
        if (requestedUrl.pathname.match(/\/mobile\/input/) && req.method == "GET") {
            action = "input";
            callController(controller_name, action, parameters);
        } else if (requestedUrl.pathname.match(/\/mobile\/create/) && req.method == "POST") {
            action = "create";
  ...
  function callController(controller_name, action, parameters) {
    ...
    if (controller) {
      controller = new controller(responseHandler, req, function() {
        controller.before_filter(action, parameters, function(err, opts) {
          if (err) {
            if (opts && opts['response_format'] == 'JSON') {
              responseHandler.renderJSON(err[0], err[1]);  
            } else {
              responseHandler.serverError(err[0], err[1]);  
            }
          } else {

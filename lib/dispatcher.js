    ...
    switch (controller_name) {
      ...
      case "devices":
          ...
          if (requestedUrl.pathname.match(/\/devices\/([0-9]+)\/new/) && req.method == "GET") {
            ...
            // devices#update
          } else if (requestedUrl.pathname.match(/\/devices\/([0-9]+)/) && req.method == "PUT") {
            action = "update";
            parameters['id'] = parts[2];
            callController(controller_name, action, parameters);

            // devices#destroy
          } else if (requestedUrl.pathname.match(/\/devices\/([0-9]+)/) && req.method == "DELETE") {
            action = "destroy";
            ...
            action = "show";
            console.log("IN DEVICE SHOW\n");
            parameters['id'] = parts[2];
            parameters['device_id'] = parts[3];
            callController(controller_name, action, parameters);
          }
          break;
      ...
      case "api":
        var path = requestedUrl.pathname.split("?")[0];
        var query_params = requestedUrl.pathname.split("?")[1];
